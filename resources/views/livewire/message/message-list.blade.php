    ...
    <div>
        <div class="sm:flex sm:items-center">
            ...
            <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                <x-button wire:click="getBalance">
                    See Balance
                </x-button>
                <x-button primary
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($messages as $message)
                                    <tr class="hover:bg-slate-100">
                                        ...
                                        <td class="px-3 py-4 text-sm text-gray-500 whitespace-nowrap">
                                            Message Send : {{ $message->sends->where('status', 'sent')->count() }} |
                                            Message Failed :
                                            {{ $message->sends->where('status', '!=', 'sent')->count() }}
                                        </td>
                                        <td class="px-3 py-4 text-sm text-gray-500 whitespace-nowrap">
            ...
            <form class="space-y-4">
                ...
                <div id="departments_list">
                    <div x-data="{ isOpen: false }"
                        ...
                        <label for="combobox"
                            class="block text-sm font-medium text-gray-700">Select Colleges</label>
        ...
        <x-modal.card wire:model.defer="view"
                ...
                <div class="px-4 py-5 border-gray-200 sm:p-0">
                    <dl class="sm:divide-y sm:divide-gray-200">
                        ...
                        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                {{ $view_details?->sends->where('status', 'sent')->count() }}/{{ $view_details?->sends->count() }}
                            </dd>
                        </div>
                        <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            <dt class="text-sm font-medium text-gray-500">
            ...
            <x-slot name="footer">
                <div class="flex space-x-3">
                    @if ($view_details?->sends->where('status', 'sent')->count() != $view_details?->sends->count())
                        <x-button wire:click="clickContinueHandler"
